...
/**
 * 
 * @brief Κλάση δυναμικόυ πίνακα που χρησιμοποιεί templates.Για την υλοποίηση συμβουλεύθηκα το βιβλίο του Stroustroup "Προγραμματισμός με τη C++".Αυτό διότι δεν εχει διδαχθεί στην ύλη η υλοποίηση ενός * vector.
 * 
...
public:
     ...
     */
    inline DynamicArray(DynamicArray<T>&& origin)
    ...
    inline typename DynamicArray<T>::reference emplace_back(Args&&... args)
    {
        ...
        */
        if (m_size == m_capacity) //Εάν το πραγματικό μέγεθος του πίνακα φτάσει το extra μεγεθος τότε πρέπει να αυξηθεί η μεταβλητή του εξτρα χώρου.
        {
        ...
        //τις παραμέτρους της σε μία άλλη συνάρτηση χωρίς να χάσει την lvalue/rvalue ιδιότητα της και αυτό γίνεται χρησιμοποιόντας την
        //forward().Στη συνέχεια μέσω της move τη μετατρέπουμε σε rvalue.Τέλος εκχωρούμε το αποτελεσμα αυτο,αφού γίνει cast σε Τ στο m_data και αυξάνουμε το μέγεθος των δεδομένων κατα 1.
    }
    /**
    ...
    void move_storage(T* dest, T* from, size_type n)
    {
        ...
        } else if (dest > from) { //Ακριβώς η ίδια λειτουργία μόνο που αυτή τη φορά λειτουργεί κάπως ανάποδα.
